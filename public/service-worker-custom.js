...
workbox.routing.registerRoute(
  new RegExp("http://localhost:3001/authors/$"),
  event => {
...
workbox.routing.registerRoute(
  ...
  event => {
    return fetch(event.url)
      ...
      .catch(async err => {
        const slug = event.url.search.substring(
          event.url.search.lastIndexOf("=") + 1
        );
        return await localforage.getItem(event.url.pathname).then(body => {
  ...
  return test.json().then(body => {
    return fetch(event.url, {
      ...
      .catch(err => {
        ...
        const id = parseInt(
          event.url.pathname.substring(event.url.pathname.lastIndexOf("/") + 1),
          10
        );
